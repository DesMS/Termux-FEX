...
if [[ ! -f "${INSTALL_DIR}/debian-arm64/debug/fex_installed" || "$1" == "update" ]]; then
  ...
  # Apply CPU spoof
  sed -i "$(grep -m1 -n "\"model name\\\\t:" "${INSTALLDIR}/debian-arm64/fex-install/Source/Tools/LinuxEmulation/LinuxSyscalls/EmulatedFiles/EmulatedFiles.cpp" | cut -d: -f1)s/.*/cpu_stream << \"model name\\\\t: ${CPU_MODEL}\" << std::endl;/" "${INSTALLDIR}/debian-arm64/fex-install/Source/Tools/LinuxEmulation/LinuxSyscalls/EmulatedFiles/EmulatedFiles.cpp"
